 ...
 */
package org.osate2.bayes.dtbn.Inference.LBP;
...
public class LoopyBeliefPropagation extends AbstractInferer {
    ...
    @Override
    public void setNetwork(final DTBN bn) {
        super.setNetwork(bn);
        this.net = bn;
        for (final DTBNNode n : bn.getNodes()) {
        ...
        for (final DTBNNode n : bn.getNodes()) {
            ...
            for (final DTBNNode c : n.getChildren()) {
    ...
    private void prepareOwnFactor(final DTBNNode n, final Edge own) {
    ...
    private Edge addEdgeToOwnFactor(final DTBNNode n, final List<Edge> adjacency) {
    ...
    public void setEvidence(final Map<DTBNNode, String> evidence) {
        beliefsValid = false;
        for (final DTBNNode n : net.getNodes()) {
    ...
    private void updateFactor(final int index) {
        for (final Edge e : transponedGraph.get(index)) {
            e.dirty = false;
            final DTBNNode n = net.getNode(e.target);
