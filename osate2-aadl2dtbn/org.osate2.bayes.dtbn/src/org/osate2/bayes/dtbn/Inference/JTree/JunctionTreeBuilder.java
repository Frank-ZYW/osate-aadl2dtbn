...
import org.osate2.bayes.dtbn.Inference.util.UnionFind;
import org.osate2.bayes.dtbn.Inference.util.triangulation.GraphElimination;
import org.osate2.bayes.dtbn.Inference.util.triangulation.IEliminationHeuristic;
...
public class JunctionTreeBuilder {
    ...
    public JunctionTree buildJunctionTree(DTBN net) {
    ...
    private Graph buildMoralGraph(DTBN net) {
    ...
    private void addMoralEdges(Graph moral, final DTBNNode node) {
        final ListIterator<DTBNNode> it = node.getParents().listIterator();
        while (it.hasNext()) {
            final DTBNNode parent = it.next();
            final ListIterator<DTBNNode> remainingParentsIt = node.getParents().listIterator(it.nextIndex());
            while (remainingParentsIt.hasNext()) { // connect parents
                final DTBNNode otherParent = remainingParentsIt.next();
    ...
    private double[] weightNodesByOutcomes(DTBN net) {
        double[] weights = new double[net.getNodes().size()];
        for (DTBNNode node : net.getNodes()) {
    ...
    private List<Pair<Edge, List<Integer>>> computeSepsets(JunctionTree junctionTree, DTBN net) {
    ...
    private final class SepsetComparator implements Comparator<Pair<Edge, List<Integer>>> {
        ...
        public SepsetComparator(DTBN net) {
            this.net = net;
        }
