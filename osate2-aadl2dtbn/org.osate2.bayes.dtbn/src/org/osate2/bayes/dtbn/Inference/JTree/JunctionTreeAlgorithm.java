 ...
 */
package org.osate2.bayes.dtbn.Inference.JTree;

import static org.osate2.bayes.dtbn.Inference.util.Pair.newPair;
...
import org.osate2.bayes.dtbn.Inference.AbstractInferer;
...
import org.osate2.bayes.dtbn.Inference.factor.arraywrapper.IArrayWrapper;
import org.osate2.bayes.dtbn.Inference.util.ArrayUtils;
import org.osate2.bayes.dtbn.Inference.util.Graph;
import org.osate2.bayes.dtbn.Inference.util.Graph.Edge;
import org.osate2.bayes.dtbn.Inference.util.MathUtils;
import org.osate2.bayes.dtbn.Inference.util.NumericalInstabilityException;
import org.osate2.bayes.dtbn.Inference.util.Pair;
import org.osate2.bayes.dtbn.Inference.util.sharing.CanonicalArrayWrapperManager;
import org.osate2.bayes.dtbn.Inference.util.sharing.CanonicalIntArrayManager;
import org.osate2.bayes.dtbn.Inference.util.triangulation.MinFillIn;
...
public class JunctionTreeAlgorithm extends AbstractInferer {
    ...
    private void incorporateAllEvidence() {
        ...
        for (DTBNNode n : evidence.keySet()) {
            incorporateEvidence(n);
        }

    }

    private void incorporateEvidence(final DTBNNode node) {
        int n = node.getId();
        isObserved[n] = true;
    ...
    @Override
    public void setNetwork(final DTBN net) {
    ...
    private JunctionTree buildJunctionTree(DTBN net) {
    ...
    private int[] computeHomeClusters(DTBN net, final List<List<Integer>> clusters) {
        int[] homeClusters = new int[net.getNodes().size()];
        for (final DTBNNode node : net.getNodes()) {
    ...
    private List<Integer> getNodeAndParentIds(final DTBNNode n) {
        final List<Integer> nodeAndParents = new ArrayList<Integer>(n.getParents().size() + 1);
        nodeAndParents.add(n.getId());
        for (final DTBNNode p : n.getParents()) {
            nodeAndParents.add(p.getId());
        }
        return nodeAndParents;
    }

    private void initializeClusterFactors(DTBN net, final List<List<Integer>> clusters, int[] homeClusters) {
    ...
    private Map<Integer, List<AbstractFactor>> findMultiplicationPartners(DTBN net, int[] homeClusters) {
    ...
    private void multiplyCPTsIntoPotentials(DTBN net, int[] homeClusters) {
        for (final DTBNNode node : net.getNodes()) {
